...
(逆向きチャネルの反証、$4.4\sigma$ の数値確定)、(iii) 静的極限とボンド希釈
...
first-order excess over the branching bound, certified numerically at
...
model satisfies $\E[N]>1/(1-\bar m)$, where $N$ is the total number of
transmission events. Consequently, \emph{the SIR-type
...
If $r(K)<1$, the expected total number of transmission events is finite and
...
\caption{Reverse channel (experiment E8c): histogram of the total number of
transmission events $N$ over $10^4$ realizations in the counterexample
regime. The measured mean exceeds the branching bound by $4.4\sigma$.
...
of Appendix~A; the resolution log is included in the artifact bundle.
...
\subsection{Contents}
The artifact repository accompanying this preprint contains:
...
the artifact repository, file \texttt{docs/phase-r/claims.md}.]
...
\texttt{docs/phase-r/c2-proof-final.md} in the artifact repository, whose
