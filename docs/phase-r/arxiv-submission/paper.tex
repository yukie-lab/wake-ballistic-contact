...
\large (Preprint --- Zenodo v2, doi:10.5281/zenodo.21979354; v1: doi:10.5281/zenodo.21955413)}
...
means the set of living marked particles is non-empty at arbitrarily large
times.
...
If $\Ts=\infty$ and $\nu$ is isotropic with unbounded support, then the
front radius satisfies $R(t)/t\to\infty$ almost surely.
\end{namedthm}
...
at arbitrarily large times, and lifetimes only govern how long the predetermined
...
(geometrically, $v$ restricted to the window box of its child branch; the
eight-branch premise: the bad set is certified separately for each of the
$2^3=8$ branch-sign combinations, namely the candidate's child branch and
the $\pm$ branches of the two parents $P$ and $P'$, taking the worst case,
as in the certifier of Appendix~\ref{app:cert}), the certified
...
\emph{every} time --- stronger than the definition of survival
(non-emptiness at arbitrarily large times, Section~\ref{sec:intro}) --- and
the full model survives. \emph{Ignition}: from the
...
Assume $\Ts=\infty$, $p>0$, and that $\nu$ is isotropic with unbounded
speed support. Fix $W>0$ with
...
surely. \qed\ Whether the isotropy hypothesis can be removed is unknown.
