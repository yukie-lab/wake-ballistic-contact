...
def scan(d_max, t_max):
    """チャンク走査: 星ごとのイベント数(d<d_max, |t|<t_max, edge除外)+ (t,d) 列"""
    ...
    for f in files:
        ...
        r, c = np.nonzero(m)
        for gi, cnt in zip(*np.unique(z["star_idx"][r], return_counts=True)):
            n_by_star[int(gi)] = n_by_star.get(int(gi), 0) + int(cnt)
        ts.append(t_ph[r, c].astype(float))
        ds.append(d_ph[r, c].astype(float))
    return n_by_star, np.concatenate(ts), np.concatenate(ds)
...
def main():
    ...
    lines = ["# 三規約交差検証(追加指示 2026-08-17 §2)", "",
             ...
             f"| 規約1 本機構(統制領域・IPW・星別露出) | **{lam1:.1f}** | — | "
             f"±10 Myr 窓(G1: ±7.0 の星母集団誤差) |",
             f"| 規約2 BJ+18 再構成(±5 Myr・1/C(t,d)・二乗則外挿) | **{lam2:.1f} ± "
             f"{sig2:.1f}** | BJ+18: 19.7±2.2 | n_enc={n_enc:.1f}, n_cor={n_cor:.0f} "
             f"(補正なしなら {lam2_raw:.1f}) |",
             f"| 規約3 FP26 再構成(±0.47 Myr・d<1 直接・÷0.70) | **{lam3:.1f}** | "
             f"FP26: 10.6±4.5(星あたり平均)/ 太陽自身 9.1 | 期待遭遇 "
             f"{n_fp:.2f} 件/0.94 Myr(補正なしなら {lam3_raw:.1f}) |", "",
             "## 判定(指示 §2)", "",
             "- 規約3 が FP26 の 9–11 級に接近するか: "
             f"**{lam3:.1f}** → " + ("接近(分裂=会計規約差を強く支持)"
                                     if 6.0 <= lam3 <= 15.0 else
                                     "乖離(会計以外の成分が残る — 帰属分解要)"), "",
