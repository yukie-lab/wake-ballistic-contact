...
def main():
    ...
    lines += ["", "**推定量の注記**: λ は星ごとの有効露出 e_i = 2·min(T, t_h_i) で"
              ...
              f"/2T 規約(±5 Myr, d<1)の再構成値 = **{lam_fl:.1f} /Myr** — "
              "一致すればファーストルックの規約が同定される。この規約は遠 |t| の"
              "検出不完全性(高 rv_error 星ほどホライズンが短い)を補正しないため"
              "下方バイアスを持ち、露出正規化(本推定量)がそれを補正して "
              "~20 /Myr に到達する。**補正深度仮説の再構成**: BJ 型 C(t,d) の"
              "『深い補正』は同じ t 方向不完全性を完備性側から直す別経路であり、"
              "C(t,d) 参考表示層のタスクで両経路の一致(→ BJ 19.7±2.2)を検証する。", ""]
