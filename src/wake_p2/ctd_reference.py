"""C(t,d) 参考表示層+補正深度仮説の検証(Phase 2 出口タスク / 裁定ログ#8(1))

位置づけ: **GeDR3mock/GDR2mock 由来の参考表示層**(明示ラベル付き)。
排除地図の条件文には入れない(案A 純度維持 — 裁定ログ#8)。

補正深度仮説(2026-08-16 指示、G1 で再構成された形):
  ファーストルック 11.7 /Myr(IPW のみ・ホライズン無視・/2T)は t 方向の
  検出不完全性で下方バイアスを持つ。これを直す2経路が一致するか:
    経路A: IPW × 星別露出正規化(統制領域)          — G1 の主推定量
    経路B: BJ 型 C(t,d)(GDR2mock 完備性)を生カウントに適用(IPW なし)
  検証項目: A ≈ B ≈ BJ 系公刊値(19.7〜21.6 /Myr @1pc)か。
...
def main():
    ...
    lines += [
        ...
        "**判定**: 経路A(観測側 IPW+露出)と経路B(mock 完備性)が"
        "独立の補正哲学で同水準に収束するか — 上表を根拠に本文で判定。", "",
